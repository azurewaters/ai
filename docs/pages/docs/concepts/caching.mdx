...
Each stream helper for each provider has special lifecycle callbacks you can use. The one of interest is likely `onFinal`, which is called when the stream is closed. This is where you can cache the full response.
